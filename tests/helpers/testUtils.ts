...
import { mockSlackWebClient, mockPerformanceMonitor, mockAIAnalytics } from '../mocks/slackApi.mock';
...
export class TestHelpers {
  static setupMockSlackClient(responses: any = {}) {
    ...
    Object.entries(responses).forEach(([category, methods]) => {
      const categoryObj = mockSlackWebClient[category as keyof typeof mockSlackWebClient];
      if (categoryObj && typeof categoryObj === 'object') {
        Object.entries(methods as Record<string, any>).forEach(([method, response]) => {
          const mockMethod = (categoryObj as any)[method];
          if (jest.isMockFunction(mockMethod)) {
            if (response instanceof Error) {
              (mockMethod as jest.MockedFunction<any>).mockRejectedValue(response);
            } else {
              (mockMethod as jest.MockedFunction<any>).mockResolvedValue(response);
            }
          }
        });
      }
